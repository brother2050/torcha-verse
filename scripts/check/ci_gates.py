...
def _run_hardcoding_gate() -> GateResult:
    ...
    """
    from scripts.check.hardcoding import main as hc_main
...
def _run_placeholder_gate() -> GateResult:
    ...
    from scripts.check.placeholders import main as ph_main
    exit_code = ph_main([])
    if exit_code == 0:
...
def _run_degrade_logging_gate() -> GateResult:
    ...
    """
    from scripts.check.degrade_logging import main as dl_main
    exit_code = dl_main([])
    if exit_code == 0:
