...
**重启条件(任一)**
1. 用户报"某个降级路径吞了错" ≥ 1 个 → 立即定位 + 补该文件
2. 决定开 `degrade_logging` CI gate (`pyproject.toml`
   `[tool.torcha-verse.ci-gates.degrade_logging] enabled = true`)
   → 必须先把 38 处降到 0
3. weekly PR 节奏: 每周一批 5-8 处, 5-8 周清完

**已经识别但未修的 38 处**(分布, 详见 `docs/ROADMAP.md` D3 阶段三):
- nodes/export.py (5), models/source/huggingface.py (4),
  consistency/score.py (3), infrastructure/config_center.py (3),
  models/source/cache.py (3), serving/app.py (3),
  tools/python_executor.py (3), assets/store.py (2),
  infrastructure/checkpoint_manager.py (2),
...
**重启时要做的事**
1. 按 `docs/ROADMAP.md` D3 阶段三 段的"快速查看" 命令查当前剩余:
   `python scripts/check_degrade_logging.py --stats`
2. 每周一批: 给最重灾文件 (nodes/export.py, models/source/huggingface.py)
   优先补 `logger.warning("...", exc_info=True)` 或改 `safe_call`
3. 38 处降到 0 后, 翻 `pyproject.toml` 的
   `[tool.torcha-verse.ci-gates.degrade_logging] enabled = true`,
   `scripts/check_ci_gates.py` 全局跑通即正式启用 CI 闸口
