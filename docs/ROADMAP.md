...
**当前统计** (silent degrade 分布, 38 处):
| 出现次数 | 文件 |
|---:|---|
| 5 | `nodes/export.py` |
| 4 | `models/source/huggingface.py` |
| 3 | `consistency/score.py` |
| 3 | `infrastructure/config_center.py` |
| 3 | `models/source/cache.py` |
| 3 | `serving/app.py` |
| 3 | `tools/python_executor.py` |
| 2 | `assets/store.py` |
| 2 | `infrastructure/checkpoint_manager.py` |
| 2 | `rag/loaders/document_loader.py` |
| 2 | `training/sft_trainer.py` |
| 1 | `consistency/scene.py` |
| 1 | `infrastructure/device_manager.py` |
| 1 | `models/providers/tiny_transformer.py` |
| 1 | `nodes/_helpers.py` |
| 1 | `plugins/manager.py` |
| 1 | `security/sandbox.py` |

**第二批重启条件** (任一):
1. 用户报"某个降级路径吞了错" ≥ 1 个 → 立即补该文件
2. 决定开 `degrade_logging` CI gate → 必须先把 38 处降到 0
