"""Numeric / ref coercion helpers shared by the L4 node modules.

These small helpers were previously duplicated across
:mod:`nodes.image`, :mod:`nodes.video`, :mod:`nodes.audio` and
:mod:`nodes.consistency`.  They are kept in their own sub-module
so that the (much larger) backend registry in
:mod:`._backends` can evolve without dragging them along.

Key behaviour:

* :func:`coerce_dim`, :func:`coerce_int`, :func:`coerce_float`
  reject ``bool`` (a subclass of :class:`int` in Python that is
  not a valid dimension) and require integer-like numbers.
* :func:`ref_id` flattens an :class:`AssetRef` / raw id string /
  id-bearing object to the underlying asset id.
"""
...
def coerce_int(value: Any) -> Optional[int]:
    ...
    Alias of :func:`coerce_dim`; kept as a separate name for readability
    at call sites that are not specifically about image / video
    dimensions.
    """
