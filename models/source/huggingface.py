...
# ---------------------------------------------------------------------------
# OpenAI-compat transport (Bearer-auth + JSON, no external deps)
# ---------------------------------------------------------------------------
class OpenAICompatTransport(HttpTransport):
    """HTTP transport that talks to OpenAI-compatible model providers.

    The OpenAI REST API is a de-facto industry standard -- the same
    shape is served by a long list of providers (Together, Anyscale,
    Fireworks, OpenRouter, vLLM with ``--served-model-name``, local
    llama-cpp-python servers, etc.).  This transport is a thin
    :class:`UrllibTransport` that injects an ``Authorization:
    Bearer <api_key>`` header by default and accepts the same
    payload as the OpenAI REST API.
    ...
    caller wants the adapter to talk to an OpenAI-compatible mirror
    instead of the canonical HF / Civitai endpoints.
    ...
    # The OpenAI-compat transport is stateless from the auth point
    # of view: it injects Bearer + Accept on every call.  Callers
    # may still pass extra headers (e.g. ``OpenAI-Organization``)
    # via the per-call ``headers`` argument.
    def _request(
        self, url: str, headers: Optional[Dict[str, str]] = None
    ) -> Tuple[bytes, Dict[str, str]]:
        hdrs: Dict[str, str] = {
...
# ---------------------------------------------------------------------------
# Ollama transport (no-auth JSON; talks to a local daemon by default)
# ---------------------------------------------------------------------------
class OllamaTransport(HttpTransport):
    """HTTP transport that talks to a local or remote Ollama daemon.

    Ollama is a popular local model runtime that exposes a JSON HTTP
    API on the loopback address by default (default URL
    ``http://127.0.0.1:11434``).  The API is largely self-describing
    and is **not** OpenAI-compatible in the v0.4.x line, so this
    transport is shipped as a separate :class:`HttpTransport`
    implementation rather than an :class:`OpenAICompatTransport`
    variant.
